...
Run `deno run --allow-net=jsr.io example.ts`. Version 0.2.0 requires an explicit
Wasm URL or bytes for the first conversation; subsequent conversations share
the initialized module. JSR exports the JavaScript
entrypoint; the explicit Wasm subpath in the npm/Vite example below belongs
to the npm package.
